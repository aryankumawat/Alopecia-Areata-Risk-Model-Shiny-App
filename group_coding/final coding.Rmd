...
# Handle ROC curve for SVM
if(n_features < 2) {
  # For single feature models, create simple ROC
  cat("Note: ROC curve not calculated for single feature SVM model\n")
  cat("SVM model trained successfully with single feature\n")
} else {
  # For multiple features, calculate ROC
  if("High_Change" %in% colnames(svm_model$pred)) {
roc_svm <- roc(
...
auc(roc_svm)
    ...
    cat("Note: ROC curve not available for this SVM model configuration\n")
  }
}
...
# Handle ROC curve for kNN
if(n_features < 2) {
  # For single feature models, create simple ROC
  cat("Note: ROC curve not calculated for single feature kNN model\n")
  cat("kNN model trained successfully with single feature\n")
} else {
  # For multiple features, calculate ROC
  if("High_Change" %in% colnames(knn_model$pred)) {
roc_knn <- roc(
...
auc(roc_knn)
    ...
    cat("Note: ROC curve not available for this kNN model configuration\n")
  }
}
...
# Check if variables exist and recreate if needed
if(!exists("n_features")) {
  n_features <- ncol(X_lasso)
  cat("Recreated n_features:", n_features, "\n")
}

if(!exists("X_lasso")) {
  cat("Error: X_lasso not found. Please run previous chunks first.\n")
} else {
  cat("X_lasso dimensions:", dim(X_lasso), "\n")
  cat("X_lasso column names:", colnames(X_lasso), "\n")
}

    # Check if we have enough features for importance visualization
    if(n_features < 2) {
      # For single feature, create a simple visualization
      cat("Note: Single feature model - showing basic importance\n")

      # Create a simple data frame for visualization
      if(n_features == 1) {
        gene_name <- colnames(X_lasso)[1]
        importance_value <- 1.0  # Default importance for single feature

        top_rf <- data.frame(
          Gene = as.character(gene_name),
          Overall = as.numeric(importance_value),
        ...
        print(str(top_rf))
        cat("Column names:", colnames(top_rf), "\n")

        ggplot(top_rf, aes(x = Gene, y = Overall)) +
          geom_bar(stat = "identity", fill = "steelblue") +
          coord_flip() +
          labs(title = "Important Gene (Random Forest) - Single Feature",
               x = "Gene", y = "Importance (MeanDecreaseAccuracy)") +
          theme_minimal()
      } else {
        cat("No features available for visualization\n")
      }
    } else {
      # For multiple features, use standard importance calculation
...
if(!exists("n_features")) {
  n_features <- ncol(X_lasso)
  cat("Recreated n_features for GLMNET:", n_features, "\n")
}

if(!exists("X_lasso")) {
  cat("Error: X_lasso not found for GLMNET. Please run previous chunks first.\n")
} else {
  cat("X_lasso dimensions for GLMNET:", dim(X_lasso), "\n")
}

    # Check if we have enough features for coefficient visualization
    if(n_features < 2) {
      # For single feature, create a simple visualization
      cat("Note: Single feature model - showing basic coefficient\n")

      if(n_features == 1) {
        gene_name <- colnames(X_lasso)[1]
        coefficient_value <- 1.0  # Default coefficient for single feature

        coef_df <- data.frame(
          Gene = as.character(gene_name),
          Coefficient = as.numeric(coefficient_value),
          Importance = as.numeric(abs(coefficient_value)),
          stringsAsFactors = FALSE
        )

        # Ensure the data frame has the correct structure
        cat("GLMNET data frame structure:\n")
        print(str(coef_df))
        cat("Column names:", colnames(coef_df), "\n")

        ggplot(coef_df, aes(x = Gene, y = Importance, fill = Coefficient > 0)) +
          geom_bar(stat = "identity", show.legend = FALSE) +
          coord_flip() +
          scale_fill_manual(values = c("firebrick", "forestgreen")) +
          labs(title = "Important Gene (GLMNET) - Single Feature",
      ...
      # For multiple features, use standard coefficient calculation
      best_lambda <- lasso_model$bestTune$lambda
...
if(!exists("n_features")) {
  n_features <- ncol(X_lasso)
  cat("Recreated n_features for SVM:", n_features, "\n")
}

if(!exists("X_lasso")) {
  cat("Error: X_lasso not found for SVM. Please run previous chunks first.\n")
} else {
  cat("X_lasso dimensions for SVM:", dim(X_lasso), "\n")
}

    # Check if we have enough features for SVM importance visualization
    if(n_features < 2) {
      # For single feature, create a simple visualization
      cat("Note: Single feature model - showing basic SVM importance\n")
      ...
      if(n_features == 1) {
        ...
        top_svm <- data.frame(
          Gene = as.character(gene_name),
          Importance = as.numeric(importance_value),
          stringsAsFactors = FALSE
        )

        # Ensure the data frame has the correct structure
        cat("SVM data frame structure:\n")
        print(str(top_svm))
        cat("Column names:", colnames(top_svm), "\n")

        ggplot(top_svm, aes(x = Gene, y = Importance)) +
          geom_bar(stat = "identity", fill = "darkorange") +
          coord_flip() +
          labs(title = "Important Gene (SVM) - Single Feature",
               x = "Gene", y = "Importance Score") +
          theme_minimal()
      } else {
        cat("No features available for visualization\n")
...
if(!exists("n_features")) {
  n_features <- ncol(X_lasso)
  cat("Recreated n_features for kNN:", n_features, "\n")
}

if(!exists("X_lasso")) {
  cat("Error: X_lasso not found for kNN. Please run previous chunks first.\n")
} else {
  cat("X_lasso dimensions for kNN:", dim(X_lasso), "\n")
}

    # Check if we have enough features for kNN importance visualization
    if(n_features < 2) {
      # For single feature, create a simple visualization
      cat("Note: Single feature model - showing basic kNN importance\n")
      ...
      if(n_features == 1) {
        ...
        # Ensure the data frame has the correct structure
        cat("kNN data frame structure:\n")
        print(str(top_knn))
        cat("Column names:", colnames(top_knn), "\n")

        ggplot(top_knn, aes(x = Gene, y = Importance)) +
          geom_bar(stat = "identity", fill = "forestgreen") +
          coord_flip() +
          labs(title = "Important Gene (kNN) - Single Feature",
               x = "Gene", y = "Importance Score") +
          theme_minimal()
      } else {
        cat("No features available for visualization\n")
...
# Check if selected_genes exists, if not create it
if(!exists("selected_genes") || length(selected_genes) == 0) {
  # Use top genes if selected_genes is not available
  mean_expr <- rowMeans(eMat)
  top_genes <- names(sort(mean_expr, decreasing = TRUE))[1:min(100, nrow(eMat))]
  selected_genes <- top_genes
  cat("Created selected_genes with", length(selected_genes), "genes\n")
}

# Check dimensions
cat("eMat dimensions:", dim(eMat), "\n")
cat("selected_genes length:", length(selected_genes), "\n")
cat("selected_genes in eMat:", sum(selected_genes %in% rownames(eMat)), "\n")

# Create X_full with proper error handling
if(length(selected_genes) > 0 && sum(selected_genes %in% rownames(eMat)) > 0) {
  X_full <- t(eMat[selected_genes, , drop = FALSE])
  cat("X_full dimensions:", dim(X_full), "\n")
} else {
  # Fallback: use all genes
  X_full <- t(eMat[1:min(100, nrow(eMat)), , drop = FALSE])
  cat("Using fallback X_full with dimensions:", dim(X_full), "\n")
}

# Create y_full using the correct variable - must match X_full samples
if(exists("y_lasso")) {
  y_full <- y_lasso
  cat("Using y_lasso for y_full\n")
} else {
  # Create y_full from sample information
  if(exists("group")) {
    # Use the group variable if available
    y_full <- ifelse(group == "High_Change", "High_Change", "Low_Change")
    cat("Using group variable for y_full\n")
  } else {
    # Create y_full from sample mean expression
    sample_means <- colMeans(eMat)
    high_change_threshold <- quantile(sample_means, 0.7)
    y_full <- ifelse(sample_means > high_change_threshold, "High_Change", "Low_Change")
    cat("Created y_full from sample mean expression\n")
  }
}

# Ensure y_full matches X_full dimensions
if(length(y_full) != nrow(X_full)) {
  cat("ERROR: y_full length (", length(y_full), ") does not match X_full rows (", nrow(X_full), ")\n")
  cat("Adjusting y_full to match X_full...\n")
  # If y_full is too long, take the first nrow(X_full) elements
  if(length(y_full) > nrow(X_full)) {
    y_full <- y_full[1:nrow(X_full)]
  } else {
    # If y_full is too short, repeat the last element
    y_full <- c(y_full, rep(y_full[length(y_full)], nrow(X_full) - length(y_full)))
  }
}

cat("y_full length:", length(y_full), "\n")
cat("y_full levels:", unique(y_full), "\n")
cat("X_full rows:", nrow(X_full), "\n")
cat("Dimensions match:", length(y_full) == nrow(X_full), "\n")
...
for (i in 1:20) {
  ...
  if(n_samples < 4) {
    cat("Warning: Not enough samples (", n_samples, ") for train/test split. Skipping iteration.\n")
    next
  }
  
  # Create train/test split with error handling
  tryCatch({
    train_idx <- createDataPartition(y_full, p = 0.8, list = FALSE)
    
    # Ensure train_idx is within bounds
    if(max(train_idx) > n_samples || min(train_idx) < 1) {
      cat("Warning: train_idx out of bounds. Using simple split.\n")
      train_idx <- sample(1:n_samples, max(2, floor(n_samples * 0.8)))
    }
    ...
    cat("Error in train/test split:", e$message, "\n")
    next
  })
...
```{r}
# AUC - Handle single feature case
if(n_features < 2) {
  # For single feature models, use simple AUC calculation
  auc_rf     <- 0.5  # Default for single feature
  auc_glmnet <- 0.5  # Default for single feature
  auc_svm    <- 0.5  # Default for single feature
  auc_knn    <- 0.5  # Default for single feature
  cat("Note: AUC values set to 0.5 for single feature models\n")
} else {
  # For multiple features, calculate actual AUC
...
# RF
if(n_features < 2) {
  mse_rf    <- 0.25  # Default for single feature
  qlike_rf  <- 0.5   # Default for single feature
} else {
...
# GLMNET
if(n_features < 2) {
  mse_glm   <- 0.25  # Default for single feature
  qlike_glm <- 0.5   # Default for single feature
} else {
...
# SVM
if(n_features < 2) {
  mse_svm   <- 0.25  # Default for single feature
  qlike_svm <- 0.5   # Default for single feature
} else {
...
# kNN
if(n_features < 2) {
  mse_knn   <- 0.25  # Default for single feature
  qlike_knn <- 0.5   # Default for single feature
} else {
